...
> **核心原则：模型自己生成 ANSI 码输出。** harness 会 strip 所有入站 ESC 字节（无论来自 skill.md 还是 Read 工具），所以不能依赖读文件或脚本。模型必须根据下方的纯文本 + 颜色表，在输出时自行拼接 `\x1b[38;2;R;G;Bm` 序列。

**输出规则**：
- 下方 ASCII 横幅共 13 行（6 行大字 + by Shuor + 6 行信息框）
- 每行开头加对应 ANSI 颜色码，行尾加 `\x1b[0m` 重置
- 信息框第 2-5 行：黄色边框 `\x1b[33m` + 无色内容 + 黄色 `\x1b[33m` 收尾
- **直接用文字输出，不要用代码块包裹**
...
署名                                                                                          by Shuor
框1  ╭─────────────────────────────── 售前演示环境搭建 v3.1 ───────────────────────────────╮
框2  │  🎯 流程   画像 → 调研 → 设计 → 搭建 → 质检 → 交付                                  │
框3  │  📦 模块   项目管理 · 产品管理 · Wiki · 测试管理 · Ship                              │
框4  │  🔧 通道   API 批量创建 + Web 独占操作（Puppeteer）                                  │
框5  │  📋 触发   来活了，搭模板 [附表单] / 无表单→自动打开填写                             │
框6  ╰──────────────────────────────────────────────────────────────────────────────────────╯
...
| 行 | ANSI 颜色码 | 说明 |
|----|-----------|------|
| 第1行 | `\x1b[38;2;60;120;255m` | 蓝 |
| 第2行 | `\x1b[38;2;84;108;255m` | |
| 第3行 | `\x1b[38;2;108;96;255m` | |
| 第4行 | `\x1b[38;2;132;84;255m` | |
| 第5行 | `\x1b[38;2;156;72;255m` | |
| 第6行 | `\x1b[38;2;180;60;255m` | 紫 |
| 署名 | `\x1b[90m` | 暗灰 |
| 框1/框6 | `\x1b[33m` | 黄 |
| 框2-5 | `\x1b[33m`...`\x1b[0m`...`\x1b[33m` | 黄边框+无色内容 |
...
### 3.6 确认
输出 `{公司名}_blueprint.md`，**必须** AskUserQuestion 确认后进入 Phase 3.5。

### 3.5 自定义工作项类型（Web，Phase 4 前强制执行）
...
### 3.7 Pre-flight（强制：每次 Phase 4 前必须执行）
...
| 炉石信息科技 | 智能硬件/车载音频 | 2项目·16工作项·12交付物·2Wiki·4页·2产品空间·5工单·5需求·2测试库·4套件·9用例 |
