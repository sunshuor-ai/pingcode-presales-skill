...
启用: {工时登记、历史数据模拟、自定义字段、自动标签、交付物}
禁用: {工时登记、历史数据模拟...}
自定义工作项类型: {标配:工程变更申请+技术评审, 硬件包:风险项+合规检查项 / 否}
演示深度: {广度优先/深度优先}
...
2. **三分支**：
   - **命中 curated**：加载该 `<行业>.md`，其「项目类型 / 阶段门 / **工作项骨架** / 测试维度 / 术语」直接驱动 Phase 3 蓝图，`types` 块进 Phase 3.7。⚠️ **工作项骨架因行业而异**——汽车电子/医疗等用**需求驱动多级**（`阶段→需求→子需求(架构/设计)→任务`），**不要套敏捷 史诗/特性/故事**；builder 按模板骨架建。
...
#### 自动化执行（2026-06-26 重写：纯本地 Playwright + Edge 跑通，弃用旧 puppeteer `pingcode_web.js`）

> 旧 `web.createWorkItemTypesForClient`（puppeteer 启发式)与真实 ngx-tethys/CDK 不对版,已弃用。现用 `scripts/ui/` 一套纯本地 Playwright(`channel:'msedge'` + `~/.pingcode-presales-edge-profile` 复用登录态,首次手工登一次)。

**四段流水(=`fields.js`)**:
1. `build_type.js` — 路径①建工作项类型(名称+图标+分组,计数差验证);
2. `add_type_to_flow.js` — **路径②把类型加进「混合项目流程」**(新类型必须先入流程才能配字段);
3. `build_attr.js` — 在「工作项属性」全局库建字段属性(简单/单选/多选[+自定义颜色]/成员[+多成员],数据项录入=输入后**点弹窗空白失焦提交,不按 Enter**);
4. `attach_attr.js` — 把字段挂到类型(该类型「属性与视图」→添加属性→thy-select 搜+选+确定)。

**提速版** `batch_type_fields.js --type=<类型>`:单会话+不重导航,有头可看,~1 分钟/类型。

**ngx-tethys 铁律(全在 memory `pingcode-ui-automation-recipe`)**:① 只认真实鼠标/键盘(`.fill()/evaluate-click` 对下拉/搜索/数据项无效);② 数据项点空白失焦提交、别按 Enter(否则"数据项已存在");③ 行级「配置」用 `evaluate el.click()`(新类型在列表底部、坐标在屏外);④ 屏外按钮(右上"添加" x≈1749)用 `evaluate 打 data-auto 标记 + locator.click`(自动滚入);⑤ 验证看"N 属性"计数差,别信详情面板文本;⑥ 查重走 API `GET /v1/project/work_item_types`(认证必带 `grant_type=client_credentials`)。
...
async function pf() {
  const authRes = await fetch(
    // ⚠️ 必须带 grant_type=client_credentials,否则 Token 获取失败(2026-06-26 实测修正)
    `https://open.pingcode.com/v1/auth/token?grant_type=client_credentials&client_id=${args.client_id}&client_secret=${args.client_secret}`
  );
