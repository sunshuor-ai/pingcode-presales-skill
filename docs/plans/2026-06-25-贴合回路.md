...
(async()=>{
  const {token}=await api.getToken('AteiWoSWcjqx','VWqaiJmordtWkAKhmfSIktmS');
...
cd scripts && node -e "
const api=require('./pingcode_api.js');
(async()=>{const {token}=await api.getToken('AteiWoSWcjqx','VWqaiJmordtWkAKhmfSIktmS');
const p=(await api.listProjects(token)).find(x=>x.identifier==='JLALED');
...
Run:
```bash
cd scripts && node pingcode_revise.js --client_id=AteiWoSWcjqx --client_secret=VWqaiJmordtWkAKhmfSIktmS \
...
"
node pingcode_revise.js --client_id=AteiWoSWcjqx --client_secret=VWqaiJmordtWkAKhmfSIktmS --identifier=JLALED --context=_smoke_ctx.json --dry
```
