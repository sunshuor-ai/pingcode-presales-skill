...
Run: `cd scripts && node pingcode_tag_apply.js --client_id=AteiWoSWcjqx --client_secret=VWqaiJmordtWkAKhmfSIktmS --identifier=JLALED --industry=汽车电子 --dry`
Expected: 打印抽出的标签（含 配光/ADB/功能安全/诊断/UDS 等领域词）、预览贴标条数与覆盖率，**不写入 PingCode**。

- [ ] **Step 3: 实跑**

Run: `cd scripts && node pingcode_tag_apply.js --client_id=AteiWoSWcjqx --client_secret=VWqaiJmordtWkAKhmfSIktmS --identifier=JLALED --industry=汽车电子`
